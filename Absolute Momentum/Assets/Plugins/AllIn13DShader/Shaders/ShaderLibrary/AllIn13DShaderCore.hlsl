...
float4 ApplyAlphaEffects(float4 inputColor, 
	float2 uvFront, float2 uvSide, float2 uvTop, 
	float frontWeight, float sideWeight, float topWeight,
	float2 uv, float2 uv2, float sceneDepthDiff, float camDistance, float4 screenPos)
{
	float4 res = inputColor;
	
#ifdef _FADE_ON
	#if defined(_TRIPLANAR_MAPPING_ON) && defined(_FADEUVSET_TRIPLANAR) && (defined(ALLIN1_FORWARD_PASS) || defined(BUILTIN_MAIN_PASS) || defined(FORWARD_ADD_PASS) || defined(SHADOW_CASTER_PASS))
		float4 fadeTop = Fade(inputColor, uvTop);
		float4 fadeFront = Fade(inputColor, uvFront);
		float4 fadeSide = Fade(inputColor, uvTop);
		
		res.a = fadeTop.a * topWeight + fadeFront.a * frontWeight + fadeSide.a * sideWeight;
	#else
		float2 selectedUV;
...
float4 ApplyAlphaEffects(float4 inputColor, EffectsData data, float2 uv, float2 uv2, float sceneDepthDiff, float camDistance, float4 screenPos)
{
	float4 res;
#if defined(_TRIPLANAR_MAPPING_ON)
	res = ApplyAlphaEffects(inputColor, 
		UV_FRONT(data), UV_SIDE(data), UV_TOP(data), 
		UV_FRONT_WEIGHT(data), UV_SIDE_WEIGHT(data), UV_TOP_WEIGHT(data),
		uv, uv2, sceneDepthDiff, camDistance, screenPos);
#else
	res = ApplyAlphaEffects(inputColor, 
		0, 0, 0,
		0, 0, 0,
...
float4 ApplyAlphaEffects(float4 inputColor, float2 uv, float2 uv2, float sceneDepthDiff, float camDistance, float4 screenPos)
{
	float4 res = ApplyAlphaEffects(inputColor,
		0, 0, 0,
		0, 0, 0,
		uv, uv2, sceneDepthDiff, camDistance, screenPos);
