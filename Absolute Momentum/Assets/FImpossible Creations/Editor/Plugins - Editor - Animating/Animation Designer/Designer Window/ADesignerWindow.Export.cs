...
namespace FIMSpace.AnimationTools
{
    public partial class AnimationDesignerWindow : EditorWindow
    {
        ...
        public void ExportToFile(string path, bool rememberSavePath = false)
        {
            ...
            exportFramerate = TargetClip.frameRate;
            ...
            try
            {
                ...
                if (_anim_MainSet.ExportFramerate > 0)
                {
                    exportFramerate = _anim_MainSet.ExportFramerate;
                    newGeneratedClip.frameRate = exportFramerate;
                }
                ...
                int keys = Mathf.CeilToInt(TargetClip.length * _play_mod_clipLenMul * exportFramerate);
        ...
        private void ComputeBakeStepDelta(float keys)
        {
            ...
            {
                if (exportFramerate <= 0f) dt = 0.1f;
                else dt = 1f / exportFramerate;
            }


            if (exportFramerate > 5f && exportFramerate < 50f)
            {
                bakeFramerateRatio = 60f / exportFramerate;
