    ...
    public partial class AnimationDesignerWindow : EditorWindow
    {
        ...
        void DisplaySaveHeaderTab()
        {
            ...
            if (ProjectFileSave != null)
            {
                ...
                if (isAutoSave) GUI.backgroundColor = Color.green;
                if (GUILayout.Button(new GUIContent(FGUI_Resources.FindIcon("Fimp/Icons/FAutoSave"), "Enabling Auto-Save every 3 minutes"), FGUI_Resources.ButtonStyle, GUILayout.Width(22), GUILayout.Height(20)))
                {
                    int current = EditorPrefs.GetInt("FimpAutoSave", 0);
                    if (current == 1) current = 0; else current = 1;
                    EditorPrefs.SetInt("FimpAutoSave", current);
                }
        ...
        void DrawPlaybackTab()
        {
            ...
            DrawExtraTopToolsSwitchButton();

            EditorGUILayout.EndHorizontal();

            if (_TopDisplayExtraTools) DrawExtraTopToolsBar();
        ...
        public void DrawPlaybackSpeedSlider()
        {
            ...
            var rect = EditorGUILayout.GetControlRect(GUILayout.MaxWidth(40)); rect.y -= 1;
            EditorGUI.LabelField(rect, new GUIContent( "(" + System.Math.Round(animationElapsed / _play_mod_Length, 2).ToString("0.00") + ")", "Current animation preview normalized time (0-1) progress value"), EditorStyles.centeredGreyMiniLabel);
        ...
        public static bool _TopDisplayExtraTools = false;
        void DrawExtraTopToolsSwitchButton()
        {
            if (_TopDisplayExtraTools) GUI.backgroundColor = new Color(0.7f, 1f, 0.7f, 1f); else GUI.backgroundColor = new Color(0.7f, 0.7f, 0.7f, 1f);

            if (GUILayout.Button(new GUIContent(FGUI_Resources.Tex_Tweaks, "Display more tools"), FGUI_Resources.ButtonStyle, GUILayout.Width(20), GUILayout.Height(18)))
            {
                _TopDisplayExtraTools = !_TopDisplayExtraTools;
            }

            GUI.backgroundColor = Color.white;
        }

        void DrawExtraTopToolsBar()
        {
            GUILayout.Space(6);
            EditorGUILayout.BeginHorizontal();
            ...
            if (TargetClip == null || ProjectFileSave == null) GUI.enabled = false;

            if (GUILayout.Button(new GUIContent("Remove Clip Design", FGUI_Resources.Tex_Remove, "Removes clip design and variant versions for currently selected AnimationClip (Work With Clip field)"), FGUI_Resources.ButtonStyle, GUILayout.Width(160), GUILayout.Height(18)))
            {
                int index = 0;
                for (int i = 0; i < ProjectFileSave.MainSetupsForClips.Count; i++)
                {
                    if (ProjectFileSave.MainSetupsForClips[i].settingsForClip == TargetClip) { index = i; break; }
                }

                ProjectFileSave.RemoveSaveDataForClip(TargetClip);
                EditorUtility.SetDirty(ProjectFileSave);
                index -= 1; if (index < 0) index = 0;

                if (ProjectFileSave.MainSetupsForClips.Count > 0)
                    TargetClip = ProjectFileSave.MainSetupsForClips[index].settingsForClip;
                else
                    TargetClip = null;
            }
