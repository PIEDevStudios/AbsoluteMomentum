        ...
        public partial class ModificatorSet : INameAndIndex
        {
            ...
            internal void DrawTopGUI(float animProgr, ADClipSettings_Main main, int i)
            {
                ...
                if (!Foldown)
                {
                    ...
                    AnimationDesignerWindow.DrawSliderProgress(Blend * BlendEvaluation.Evaluate(animProgr), 95, 177, r);
                }
                else
                {
                    AnimationDesignerWindow.GUIDrawFloatPercentage(ref Blend, new GUIContent("Modifier Blend  "));
                    AnimationDesignerWindow.DrawSliderProgress(Blend * BlendEvaluation.Evaluate(animProgr), 95, 55);
                    AnimationDesignerWindow.DrawCurve(ref BlendEvaluation, "Blend Along Clip Time:");
                    AnimationDesignerWindow.DrawCurveProgress(animProgr, 154);
                }
            ...
            internal void DrawParamsGUI(float animProgr, AnimationDesignerSave save)
            {
                ...
                if (Type == EModification.AdditiveRotation || Type == EModification.OverrideRotation)
                {
                    ...
                    var rotRect = GUILayoutUtility.GetLastRect();

                    rotRect.width = 40;
                    if ( GUI.Button(rotRect, GUIContent.none, EditorStyles.label))
                    {
                        if ( FGenerators.IsRightMouseButton())
                        {
                            GenericMenu menu = new GenericMenu();
                            menu.AddItem(new GUIContent("Get current bone rotation angles"), false, () => { RotationValue = Transform.localEulerAngles; EditorUtility.SetDirty(save); });
                            menu.ShowAsContext();
                        }
                    }
