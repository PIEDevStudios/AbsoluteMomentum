            ...
            if (S)
            {
                ...
                if (mainSetup != null)
                {
                    ...
                    if (additionalSetupSettingsFoldout)
                    {
                        ...
                        if (mainSetup.ExportFramerate == 0f)
                        {
                            float fps = TargetClip.frameRate;
                            GUI.color = new Color(1f, 1f, 1f, 0.7f);
                            fps = EditorGUILayout.FloatField("Export Framerate:", fps);
                            GUI.color = Color.white;
                            if (fps != TargetClip.frameRate) mainSetup.ExportFramerate = fps;
                        }
                        else
                        {
                            mainSetup.ExportFramerate = EditorGUILayout.FloatField("Export Framerate:", mainSetup.ExportFramerate);
                            if (mainSetup.ExportFramerate <= 0f) mainSetup.ExportFramerate = 0f;
                            if (mainSetup.ExportFramerate > 240f) mainSetup.ExportFramerate = 240f;
                        }
                            ...
                            if (latestSecondaryAnimator != null)
                            {
                                ...
                                else
                                {
                                    if (TargetClip.length != 0f && latestSecondaryAnimatorClip.length != 0f)
                                    {
                                        ...
                                        moveSecondaryAnimator = EditorGUILayout.Toggle(new GUIContent(" Move Secondary Animator:", FGUI_Resources.Tex_Movement), moveSecondaryAnimator);
                                        EditorGUIUtility.labelWidth = 0;

                                        if (moveSecondaryAnimator)
                                        {
                                            GUILayout.Space(1);
                                            EditorGUI.indentLevel++;

                                            seconadaryAnimatorPosition = EditorGUILayout.Vector3Field("Position: ", seconadaryAnimatorPosition);
                                            seconadaryAnimatorRotation = EditorGUILayout.Vector3Field("Rotation: ", seconadaryAnimatorRotation);
